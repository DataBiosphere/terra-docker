...
if [ -n "$1" ]; then
  ...
  sudo -E -u jupyter /usr/jupytervenv/bin/jupyter nbextension install ${JUPYTER_HOME}/${JUPYTER_EXTENSION_NAME}/ --user
  sudo -E -u jupyter /usr/jupytervenv/bin/jupyter nbextension enable ${JUPYTER_EXTENSION_NAME}/main
fi
