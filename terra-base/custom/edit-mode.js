define(() => {
  ...
  function updateParams() {
    ...
    const welderUrl = leoUrl + `/proxy/${params.googleProject}/${params.clusterName}/welder`;
  ...
  function renderFileNotTrackedBanner() {
    ...
    const toolTipText = '<p>Your changes are not being saved to the workspace.</p>';
  ...
  // this function assumes any status not included in these lists represents a notebook out of sync
  // this is done to defend against future fields being added being auto-categorized as failures
  function handleCheckMetaResp(res) {
    const healthySyncStatuses = ['LIVE'];
    // below is included for reference, but commented out so it does not cause errors with linting
    // const outOfSyncStatuses = ['DESYNCHRONIZED', 'REMOTE_CHANGED'];
  ...
  const lockConflictBody = `<p>This file is currently being edited by another user. ` +
      `Please allow 2-3 minutes after this user has closed the file for it to become available for editing.</p>` +
  ...
  function promptUserWithModal(title, buttons, htmlBody) {
    ...
    // we need to require here because otherwise we haven't checked if the module is loaded
    // (i.e. if we are in terminal view)
  ...
  function notebookSaveAs() {
    ...
    // guarantees a path in [0] and file name in [1]. [0] is "" if just a file is passed
