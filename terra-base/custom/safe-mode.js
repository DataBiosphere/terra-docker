...
define(() => {
  ...
  function updateParams() {
    ...
    const welderUrl = `/proxy/${params.googleProject}/${params.clusterName}/welder`;
  ...
  function toggleUIControls(shouldHide) {
    ...
    const selectorsToHide = ['#save-notbook', '#new_notebook',
      '#open_notebook', '#copy_notebook', '#save_notebook_as',
      '#save_checkpoint', '#restore_checkpoint', '.checkpoint_status',
      '.autosave_status', '#notification_notebook', '#file_menu > li.divider:eq(0)',
      '#file_menu > li.divider:eq(2)'
    ];
