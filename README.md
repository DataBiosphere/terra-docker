...
* Since 6/28/2021, we introduced a few changes that might impact building custom images
    - `home` directory of new images will be `/home/jupyter`. This means if your dockerfile is referencing `/home/jupyter-user` directory, you need to update it to `/home/jupyter`
    - Creating VMs with custom images will take much longer than `terra-jupyter-*` images because `docker pull` will take a few min. If the custom image ends up being too large, VM creation may time out. New base images are much larger in size than previous versions.
